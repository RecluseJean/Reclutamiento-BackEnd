...
@RequestMapping("/api")
@CrossOrigin
public class SigninPostulanteController {
    ...
    public SigninPostulanteController(AuthenticationManager authenticationManager, JwtProvider jwtProvider,
                                      IUsuarioService usuarioService) {
    ...
    @PostMapping("/postulante/signin")
    public ResponseEntity<?> SigninPostulante(@RequestBody SigninRequest signinRequest) {
        ...
        if (postulante_data.isPresent()) {
            ...
            if (userDetails.getAuthorities().contains(new SimpleGrantedAuthority("ROLE_POSTULANTE"))) {
